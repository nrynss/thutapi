...
| **Date** | 2026-09-04 |
| **Commit** | `1c61d7e` — single focused commit covering the M1 fix and tests; L1 disposition is recorded here and the line is left as-is (see below). The SHA embedded in this row can drift on rebase/amend; if it does not resolve, `git log --grep='T2 round 1 remediation'` finds the live ref for the same change. |
...
| **Commit SHA** | `1c61d7e` (the remediation commit). If a future amend drops this SHA from history, `git log --grep='T2 round 1 remediation'` recovers the live ref. |
