...
| 9 | MP4 **and PDF** → **the child** | book page, `<video>` (a film is always made — captioned-silent when narration is out) and **always** a PDF download | **T10f + T10g DONE** | `326d2c1` (PDF, round-3 APPROVE) + `7e8c5a8` (captions at 1080×1620, round-2 APPROVE) — `book_ready` carries both `pdf_url` and `video_url` on every run. The page/catch-up route is T10b's |
...
| ~~**T9**~~ | `static/**`, `internal/web/**`, route lines | — | **DONE** | Closed at round-6 **APPROVE 0/0/0/0, zero residue**; real-phone HTTPS check deferred to operator verification |
...
**~~T7~~ → ~~T8~~ → ~~T10c~~ → ~~T10d~~ → ~~T10e~~ → ~~T10f~~ → (~~T10g~~ ∥ ~~T9~~) → ~~T10b~~ → ~~T11~~ → T9b → T14**,
with ~~T10a~~, ~~T9a~~, ~~T8b~~, ~~T5c~~, ~~T12~~ and ~~T13~~ done. **Only T9b and T14 remain**, and T14 is
never cut.
...
the film has nowhere to persist, so no book has ever completed in production.
...
| **T5c** | **DONE** 2026-09-05. Closed after round-1 remediation and **round-2 APPROVE 0/0/0/0, zero residue** (t5c-round1.md, t5c-remediation-round1.md, t5c-round2.md). Aligned `story.Emotions` with GMI Speech 2.8 HD provider enum (`console.gmicloud.ai/api/v1/ie/requestqueue/apikey/models/minimax-tts-speech-2.8-hd`, t8b-live-record.md): replaced `"neutral"` with `"calm"`, documented why `"auto"` is omitted, updated byte pin in `wire_test.go`, updated fixtures in `validate_test.go` and `structure_test.go`, added enum-subset pin `TestEmotionsSubsetOfProviderEnum` asserting `"auto"` exclusion. Sanctioned contract row C1 updated out-of-set test case in `internal/audio/narrate_test.go` to `"neutral"`. Doc contract rows C2 and C3 flipped to confirmed. Coverage: story 100%. |
...
none of the history. The state itself is readable — approved pages are
`MediaIllustration` rows and the film is the book's `video/mp4` row — but
**the HTTP read that serves it on the book's own route does not exist**, and
T10c recorded it as contract row C4 rather than inventing a route outside its
`Owns`.

**It is T10b's, and it is a requirement rather than a nicety.** Without it a
reload at minute four restarts §T9a's race at `--done: 0` while the pipeline
is four pages in — the book page would tell the child less than the truth,
which is the one thing §The flow's progress rule exists to prevent. It is also
what makes a shared link work cold on a book still being made.

**It is not a separate track.** It is the same route and the same template as
the player itself; splitting it would put two tracks in one file, which is the
collision §Sequencing already names as the only real merge hotspot.

### T10b C4 consumer seam — SANCTIONED

The T10c round-1 C4 row assigns the HTTP state surface to T10b. To complete
that surface, this row sanctions the narrow integration seam below. T10c and
T9 remain owners of their files; the seam grants T10b only the C4 state
contract and its consumer adapter, with no authority to change their other
behavior. The existing T10c C4 assignment and T9's closed screen-5 contract
are the owner-consent record for this dependency.

| Owner's path | T10b responsibility | Boundary and pin |
| --- | --- | --- |
| `internal/bookgen/bookgen.go` | Expose the process-local `CatchUp` snapshot (`GenerationStatus`, `ApprovedPage`) and record approvals for the latest run so the T10b state read can distinguish running, ready, failed and unreadable (`unknown`). | C4 state only; no pipeline-stage or retry changes. `TestCatchUpReportsRunStatusAndApprovedPages` pins the status mapping and sorted approvals. |
| `internal/bookgen/pipeline.go` | Record each already-persisted approval immediately before publishing its `page_approved` event, allowing the T10b snapshot to deduplicate the from-now-on stream. | Approval ordering only; no event shape or stage behavior changes. `TestPipeline_ApprovalEventsCarryRealIds` remains the pipeline pin. |
| `internal/bookgen/bookgen_test.go` | Keep the C4 snapshot contract pin beside its producer. | Test-only coverage for the sanctioned API. |
| `static/app.js` | Adapt T9's ended-interview screen to the T10b C4 protocol: subscribe to the book stream, read `/book/{id}/state` once after `open`, merge page numbers, and leave `unknown` recoverable without a new POST. | Adapter only; T10b owns the protocol in `static/book/catchup.js`. `testEndedReloadCatchesUpRunningGeneration`, `testC4WaitsForOpenAndKeepsBoundaryEvent`, `testC4StateFailureKeepsLiveStream` and `testC4UnknownStateIsRecoverable` pin the handoff. |
| `static/browser-test.js` | Pin the adapter's C4 lifecycle and unknown-state behavior. | Test-only coverage for this seam; no unrelated T9 UI changes. |

Any future change outside these paths or outside the C4 responsibilities above
requires a new contract row from the owning track before T10b edits it.

---

## T10d — The film's content type

**Owns:** `supportedTypes` in `internal/mediastore/mediastore.go` and its test.

**Depends on:** nothing. **Done when:** a `video/mp4` blob persists and serves
back through `GET /media/{id}`, and T10c's film-persist step succeeds against
the real store.

**Assigned 2026-09-05 as T10c's contract row C2.** `mediastore.supportedTypes`
(`mediastore.go:69-75`) is a closed set built around what the image and TTS
models return — `image/png`, `image/jpeg`, `image/webp`, `audio/mpeg`,
`audio/wav`. **No video type.** The film therefore cannot be persisted, so
`book_ready {"video_url":"/media/<id>"}` cannot name a real blob and **no
production run can finish.**

**This is the last thing between the repo and its first true end-to-end run.**
Every other hop of §The generation path is proven live; hop 8's mux is proven
offline; hop 9 has never happened because the film has nowhere to land. One map
entry stands in the way.

**Why it is T10d and not T3c.** It edits `internal/mediastore`, which is T3's
`Owns` — but a letter track here is named for the track whose need created it,
not the directory it touches: **T3b** owned `internal/stream`, `internal/job`
and the queue polling inside `internal/gmi/media`, none of them T3's files. The
video type exists solely because §T10 made the book a film. T3 is complete for
its own purposes and nothing about it is unfinished.

**Why not folded into T11**, which already has a declared foothold in
`internal/mediastore` (the retention sweep): **T11 depends on T10**, and C2 has
to land *before* T10c can produce a book at all. Putting it there makes the
prerequisite depend on the thing it is a prerequisite for.

### The change

* Add `"video/mp4": true` to `supportedTypes`, and extend the doc comment above
  it — it currently explains the set as "the image types the request-queue
  image models return, Speech 2.8's mp3, and wav as the TTS fallback", which
  will be wrong the moment video is in it.
* Pin it: a `video/mp4` blob persists, and `GET /media/{id}` serves it back
  with that content type and a working Range request. Range matters here more
  than for any other type — a `<video>` element scrubs, and §T10's `+faststart`
  exists precisely so playback can begin before the download finishes.
* **Do not open the set further.** It is closed on purpose; add the one type
  the product actually produces. `video/webm` and friends are not produced by
  anything in this repo.

**The failure it removes is already loud, not silent** — `bookgen.go:134-138`
persists the film as `video/mp4` and lets the run fail at that step with
`failed {}` rather than emitting a `book_ready` naming a URL that cannot be
stored. That is the correct behaviour and should stay after this lands; T10d
removes the cause, not the guard.
...
* **When narration 503s**, the run emits `narration_unavailable {}` and still
  ...
  **Superseded in part by §T10g — activated at `7e8c5a8`:** the film is no
  longer skipped. Because the words are now on the page, a captioned film is
  made either way — silent, or over §T12's bed. So the outage costs the
  voices, not the video, and `book_ready` carries both a `pdf_url` and a
  `video_url`, always.
* `book_ready` carries both `pdf_url` and `video_url` on every run (T10g
  landed the captioned-silent tier; the run's narration may be absent, the
  film never is). §T9 and §T10b read "a film is always made; the voices may
  be resting".
