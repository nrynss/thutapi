...
| **T2** | DONE. Closed at 3695375 after 1 review round (residue: M1 closed in this remediation; L1 false-positive on inspection). Wire shape verified end-to-end: 12 → 14 subtests pass -race; live endpoint probes reached both production hosts (Cloudflare-fronted text, APISIX-fronted media) with the literal request shape, returning 401 from auth — `classifyStatus` maps to `ErrUnauthorized` correctly. Operator-side note: bearer key in `apikey.txt` is rejected by both providers (shape `sk-or-v1-…` looks like an OpenRouter key, not GMI); replace with a production GMI key before T6/T8 live-call gates. See dev-diary/adversarial-review/t2-round1.md and t2-remediation-round1.md. |
| **T3** | Not started. |
| **T4** | Not started. |
| **T5** | Not started. |
| **T6** | Not started. |
