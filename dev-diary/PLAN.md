...
| **T10f** | **DONE** 2026-09-06, closed at `326d2c1` — round 1 (0C/1H/2L) → round 2 (0C/0H/0M/5L) → **round 3 APPROVE 0/0/0/0, zero residue** (t10f-round1.md → t10f-remediation-round1.md → t10f-round2.md → t10f-remediation-round2.md → t10f-round3.md). `internal/bookpdf` renders the printable A4 book (title page + byline, page JPEG pass-through + text, Fredoka embedded single-source from the vendored variable face, fpdf v0.9.0 as a direct require with its §T0 reason); `application/pdf` joined mediastore's closed set (T10d's `video/mp4` precedent) with a Range pin; bookgen gained its PDF stage between narrate and film under contract rows; cmd wired. **Outage semantics (pre-activation contract):** narration 503 (`gmi.ErrTransient`) → `narration_unavailable {}` once, the run still **succeeds** PDF-only with `job.StatusDone`; non-transient narration failure fails the run; `book_ready` carries `pdf_url` always and `video_url` omitempty. The §T10f↔§T10g reconciliation (captioned silent film either way, both URLs always) is **activation-conditional on §T10g's implementation, which is spec-only** — annotated at §T10f; T10g's own contract rows into bookgen will flip the branch. |
