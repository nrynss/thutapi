...
| **T2** | Implementation landed at 3695375; round-1 review at the same SHA returned REMEDIATE (1M/1L). M1 (classifyStatus 4xx → ErrBadRequest) fixed at 4a176a2 with two Pin tests; L1 (docstring 'spelled volumn in GMI's API') recorded as false-positive on inspection. Awaiting round-2 review for zero-residue claim; T2 not closed until that lands. Live-endpoint note: bearer key in `apikey.txt` is rejected by both providers (shape `sk-or-v1-…` looks OpenRouter, not GMI); replace before T6/T8 live-call gates. |
| **T3** | Not started. |
| **T4** | Not started. |
| **T5** | Not started. |
| **T6** | Not started. |
