...
func (s *Store) Persist(ctx context.Context, src io.Reader, contentType string) (string, error) {
	...
	return s.persistWithID(ctx, id, src, ct)
}

// PersistWithID is the caller supplied-id variant of Persist. It lets a
// consumer reserve the id before the blob and metadata row become reachable,
// so a separate ownership record can cover an interrupted persist. The id
// must have the same 128-bit lowercase-hex shape generated by Persist.
func (s *Store) PersistWithID(ctx context.Context, id string, src io.Reader, contentType string) error {
	ct, ok := normalizeContentType(contentType)
	if !ok {
		return fmt.Errorf("mediastore: persist: %w: %q", ErrInvalidContentType, contentType)
	}
	...
	_, err := s.persistWithID(ctx, id, src, ct)
	return err
}

func (s *Store) persistWithID(ctx context.Context, id string, src io.Reader, ct string) (string, error) {
...
// DeleteIfPresent removes a blob when it exists and succeeds when the id is
// already absent. It is used by pre-registered short-lived media cleanup,
// where a crash may occur before the metadata row is inserted.
func (s *Store) DeleteIfPresent(ctx context.Context, id string) error {
