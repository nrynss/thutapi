...
| `UPLOAD_TOKEN` | **yes** | generated by deployment | Bearer for the voice-sample upload path (`POST /voice-sample`). The process refuses to start without one; `deploy/docker-run.sh` generates a random token per run. Keep it out of the browser bundle; an adult supplies it only for a consented sample. |
